# 📝 2025 Winter Moed B - Answers Sheet

Please write your answers in the designated placeholders below. When you are finished, type `Finished` or `Check my answers` in the chat to submit them for grading.

---

## Question 1: Flow Networks & Ford-Fulkerson (20 Points)

### Part a) [6 Points]
*Define augmenting path and bottleneck capacity.*

*Write your answer here:*


---

### Part b) [7 Points]
*Describe the algorithm, prove correctness, and analyze complexity.*

*Write your answer here:*


---

### Part c) [7 Points]
*Explain why cycle-based augmenting paths can fail.*

*Write your answer here:*


---

## Question 2: Randomized Permutation Generation (20 Points)

### Part a) [10 Points]
*Analyze expected and w.h.p. runtimes.*

$$
\displaylines{

}
$$


---

### Part b) [10 Points]
*Prove the uniform distribution.*

*Write your answer here:*


---

## Question 3: Minimal Feedback Arc Set (20 Points)

*Describe your algorithm, analyze complexity, and prove correctness.*

*Write your answer here:*


---

## Question 4: Forest and Tree Orientation (20 Points)

### Part a) [10 Points]
*Prove that at least one node has difference >= 1.*

*Write your answer here:*


---

### Part b) [10 Points]
*Describe the orientation algorithm for trees and explain correctness.*

*Write your answer here:*


---

## Question 5: Longest Complementary-Reverse Subsequence (20 Points)

### Part a) [10 Points]
*Describe the recursive formula and explain correctness.*

*Write your answer here:*


---

### Part b) [10 Points]
*Describe the DP algorithm and analyze complexities.*

*Write your answer here:*


---
